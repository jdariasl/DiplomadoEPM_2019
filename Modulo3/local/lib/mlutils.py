...
def show_preds(x, y, preds):
    for i in range(len(x)):
        ...
        plt.bar(np.arange(len(preds[i])), preds[i], width=0.4, color=color, alpha=.5, label=hitmiss)
#        plt.bar(np.arange(len(preds[i]))+0.5, np.eye(len(preds[i]))[int(y[i])], width=0.4, color="red", alpha=.5, label="label")
...
def print_confusion_matrix(ytrue, ypred, class_names, figsize = (10,7), fontsize=14):
